...
class PdfStorageService {
   ...
   */
  async getPdf(fileIdentifier, applicationId = null) {
    // Handle different storage patterns:
    // 1. If it's a full path, use as-is
    // 2. If applicationId is provided, look in permits/{applicationId}/
    // 3. Otherwise, look in pdfs/
    let fullIdentifier;
    
    if (path.isAbsolute(fileIdentifier) || fileIdentifier.includes('/')) {
      fullIdentifier = fileIdentifier;
    } else if (applicationId) {
      // For permits, look in permits/{applicationId}/filename
      fullIdentifier = path.join('permits', applicationId.toString(), fileIdentifier);
    } else {
      // Default to pdfs directory
      fullIdentifier = path.join(PDF_SUBDIRECTORY, fileIdentifier);
    }
