...
// Upload middleware is no longer needed since the app doesn't require document uploads
...
jest.mock('../../middleware/auth.middleware', () => ({
  isAuthenticated: (req, res, next) => {
    // Always add user data to the request for tests
    ...
    console.log('Auth middleware - Added test session data');
    next();
  },
  isAdminPortal: (req, res, next) => {
    // Always add admin user data to the request for tests
    req.session = req.session || {};
    req.session.userId = 1;
  ...
  isClient: (req, res, next) => {
    // Always add client user data to the request for tests
    req.session = req.session || {};
    req.session.userId = 1;
    req.session.userEmail = 'test@example.com';
    req.session.userName = 'Test';
    req.session.accountType = 'client';
    req.session.isAdminPortal = false;
    console.log('Client middleware - Added test session data');
    next();
  },
...
async function setupTestSession(sessionId, sessionData) {
  return new Promise((resolve, reject) => {
    ...
    store.set(sessionId, sessionData, (err) => {
      if (err) return reject(err);
      console.log(`Test session set up with ID ${sessionId}:`, sessionData);
      resolve();
    });
  });
}
