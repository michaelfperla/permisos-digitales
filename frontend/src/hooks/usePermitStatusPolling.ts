...
export const usePermitStatusPolling = ({
  ...
  const checkStatus = useCallback(async () => {
    if (!applicationId || !currentStatus) return;

    try {
      const response = await paymentService.checkPaymentStatus(applicationId);
  ...
  const startPolling = useCallback(() => {
    ...
    const poll = () => {
      ...
      if (attemptCountRef.current > config.maxAttempts) {
        console.log(`Polling stopped after ${config.maxAttempts} attempts for status: ${currentStatus}`);
        stopPolling();
        return;
      }
  ...
  useEffect(() => {
    if (shouldPoll(currentStatus)) {
      console.log(`Starting status polling for: ${currentStatus}`);
      startPolling();
    } else {
      console.log(`Stopping status polling for: ${currentStatus}`);
      stopPolling();
    }

    // Cleanup on unmount or status change
    return stopPolling;
  }, [currentStatus, shouldPoll, startPolling, stopPolling]);
