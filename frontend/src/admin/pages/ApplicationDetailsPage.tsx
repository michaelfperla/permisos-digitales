...
const ApplicationDetailsPage: React.FC = () => {
  ...
  const { data: paymentProofDetails, isLoading: isLoadingPaymentProof, isError: isPaymentProofError, error: paymentProofError } = useQuery({
    ...
    queryFn: () => getPaymentProofDetails(id!),
  ...
  const getStatusDisplayName = (status: string): string => {
    switch (status) {
      ...
      case 'PENDING_PAYMENT': return 'Pendiente de Pago (Legacy)';
      case 'PROOF_SUBMITTED': return 'Comprobante Enviado (Legacy)';
      case 'PROOF_REJECTED': return 'Comprobante Rechazado (Legacy)';
  ...
  const handleVerifyPayment = async () => {
    if (showVerifyModal) {
      try {
        ...
        const result = await verifyPayment(id!, verifyNotes);
  ...
  const handleViewPaymentProof = () => {
    if (id) {
      window.open(`/admin/applications/${id}/payment-proof-file`, '_blank');
    }
  };
  ...
  const handleDownloadPermit = () => {
    if (id) {
      window.open(`/admin/applications/${id}/download-permit`, '_blank');
    }
  };

  // Handle recommendations download
  const handleDownloadRecommendations = () => {
    if (id) {
      window.open(`/admin/applications/${id}/download-recommendations`, '_blank');
    }
  };

  // Handle PDF retry
  const [isRetrying, setIsRetrying] = useState(false);
  const handleRetryPdfGeneration = async () => {
    if (!id) return;
    
    try {
      setIsRetrying(true);
      showToast('Reintentando generación de PDF...', 'info');
      
      const response = await fetch(`/admin/applications/${id}/generate-pdf`, {
        method: 'POST',
        headers: {
      ...
      if (result.success) {
        showToast('PDF agregado a la cola de generación', 'success');
        refetch(); // Refresh application data
      } else {
        showToast(`Error: ${result.message || 'Error desconocido'}`, 'error');
      }
    } catch (error) {
      showToast('Error al reintentar generación de PDF', 'error');
    } finally {
      setIsRetrying(false);
    }
  };
    ...
    <ResponsiveContainer maxWidth="xxl">
      <header className={styles.pageHeader}>
        ...
        <div className={styles.headerActions}>
          ...
          {/* Modern permit actions */}
          {application?.status === 'PERMIT_READY' && (
            <>
              {application.permit_file_path && (
                <Button
                  variant="primary"
                  size="small"
                  icon={<Icon IconComponent={FaDownload} size="sm" />}
                  onClick={handleDownloadPermit}
                >
                  Descargar Permiso
                </Button>
              )}
              {application.recomendaciones_file_path && (
                <Button
                  variant="info"
                  size="small"
                  icon={<Icon IconComponent={FaDownload} size="sm" />}
          ...
          {application?.status === 'ERROR_GENERATING_PERMIT' && (
            <Button
              variant="warning"
              size="small"
              icon={<Icon IconComponent={isRetrying ? FaSpinner : FaRedo} size="sm" />}
              onClick={handleRetryPdfGeneration}
              disabled={isRetrying}
            >
              {isRetrying ? 'Reintentando...' : 'Reintentar PDF'}
            </Button>
          )}

          {/* Generating status */}
          {application?.status === 'GENERATING_PERMIT' && (
            <div className={styles.headerActions}>
              <Icon IconComponent={FaSpinner} size="sm" />
              <span>Generando permiso...</span>
            </div>
          )}
        </div>
      </header>
      ...
      <div className={styles.applicationContent}>
        ...
        <div className={styles.detailsSection}>
          ...
          <div className={styles.detailsCard}>
            ...
            <div className={styles.detailsGrid}>
              ...
              <div className={styles.detailItem}>
                <span className={styles.detailLabel}>Línea/Versión/Submarca:</span>
                <span className={styles.detailValue}>{application?.linea}</span>
              </div>
              <div className={styles.detailItem}>
                <span className={styles.detailLabel}>Modelo/Año:</span>
              ...
              <div className={styles.detailItem}>
                <span className={styles.detailLabel}>Número de Serie (VIN/NIV):</span>
        ...
        {(application?.status === 'GENERATING_PERMIT' || 
          application?.status === 'ERROR_GENERATING_PERMIT' ||
          (application?.processing?.pdfAttemptCount ?? 0) > 0) && (
          <div className={styles.detailsSection}>
            <h2 className={styles.sectionTitle}>Estado de Procesamiento</h2>
            <div className={styles.detailsCard}>
              <div className={styles.detailsHeader}>
                <Icon IconComponent={FaClock} className={styles.detailsIcon} size="lg" />
                <h3 className={styles.detailsTitle}>Generación de PDF</h3>
              </div>
              <div className={styles.detailsGrid}>
                <div className={styles.detailItem}>
                ...
                {(application?.processing?.pdfAttemptCount ?? 0) > 0 && (
                  <div className={styles.detailItem}>
                    <span className={styles.detailLabel}>Intentos de Generación:</span>
                    <span className={styles.detailValue}>{application?.processing?.pdfAttemptCount}</span>
                  </div>
                )}
                {application?.processing?.lastPdfAttempt && (
                  <>
                    <div className={styles.detailItem}>
                      <span className={styles.detailLabel}>Último Intento:</span>
                      <span className={styles.detailValue}>
                        {formatDate(application.processing.lastPdfAttempt.created_at)}
                      </span>
                    </div>
                    {application.processing.lastPdfAttempt.status && (
                      <div className={styles.detailItem}>
                        <span className={styles.detailLabel}>Estado del Último Intento:</span>
                        <span className={styles.detailValue}>
                          {application.processing.lastPdfAttempt.status}
                        </span>
                      </div>
        ...
        {/* Files Section */}
        {(application?.status === 'PERMIT_READY' || application?.status === 'COMPLETED') && (
          <div className={styles.detailsSection}>
            <h2 className={styles.sectionTitle}>Archivos Generados</h2>
            <div className={styles.detailsCard}>
              <div className={styles.detailsHeader}>
                <Icon IconComponent={FaFileAlt} className={styles.detailsIcon} size="lg" />
                <h3 className={styles.detailsTitle}>Documentos del Permiso</h3>
              </div>
              <div className={styles.paymentProofActions}>
                ...
                {!application.permit_file_path && !application.recomendaciones_file_path && (
